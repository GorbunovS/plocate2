<template>
  ...
  <div class="body gap-5 flex flex-col min-h-screen text-gray-900 dark:text-gray-100">
    ...
    <div v-if="currentPage === 'home'"
      class="views flex flex flex-col items-center justify-center text-center p-4 gap-4">
      ...
      <div class="actions p-4 flex flex-col gap-2 sm:gap-4 w-full justify-center mt-15">
        <Button label="Создать объявление" severity="success" variant="outlined" @click="currentPage = 'newAd'"
          class=" sm:w-auto" />
...
const currentTheme = ref(Telegram.WebApp.colorScheme || 'light');


const updateTheme = () => {
  currentTheme.value = Telegram.WebApp.colorScheme;
};

// Подписка на событие изменения темы
onMounted(() => {
  Telegram.WebApp.onEvent('themeChanged', updateTheme);
  updateTheme(); // Инициализация при монтировании
});

// Отписка при размонтировании
onUnmounted(() => {
  Telegram.WebApp.offEvent('themeChanged', updateTheme);
});

// Computed для источника логотипа (обновлённая версия вашей функции)
const logoSrc = computed(() => {
  return currentTheme.value === 'light' ? Pl_logo : Pl_logo_dark;
});
